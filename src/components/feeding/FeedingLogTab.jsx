...
import { RotateCcw, Trash2, Calendar, User, Clock, Plus, Filter, Tag, Zap, Droplets } from 'lucide-react';
...
export const FeedingLogTab = ({
  ...
  const [activeLogTab, setActiveLogTab] = useState('ALL'); // 'ALL', 'FEED', 'AERATOR'
  ...
  const getTriggerBadge = (notes = '') => {
    ...
    return (
      <span className={`px-2 py-0.5 text-[9px] font-extrabold rounded-md border ${style}`}>
        {label}
      </span>
    );
  };
  ...
  return (
    <div className="space-y-6 animate-[fadeIn_0.4s_ease-out]">
      {/* ── TOP SECTION: INTERACTIVE HARDWARE CONTROL PANEL (AERATOR 24H & SMART FEEDER) ── */}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white border border-slate-200 rounded-xl p-5 shadow-sm overflow-hidden flex flex-col">
          <div className="border-b border-slate-100 pb-3 mb-4 flex items-center justify-between">
            <div>
              <h2 className="text-sm font-semibold text-slate-800 uppercase tracking-wider">Riwayat Pemberian Pakan & Audit Trail</h2>
